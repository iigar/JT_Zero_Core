...
export default function Sidebar({ state, threads, engines }) {
  ...
  return (
    <aside
      data-testid="sidebar"
      className="flex flex-col gap-3 p-3 bg-[#0D1017] border-r border-[#2D3A4E] overflow-y-auto"
    >
      {/* System */}
      <div>
        <h3 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 font-semibold">System</h3>
      ...
      <div className="border-t border-[#2D3A4E]" />

      {/* Sensors */}
      <div>
        <h3 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 font-semibold">Sensors</h3>
      ...
      {/* Threads */}
      <div>
        <h3 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 font-semibold">Threads</h3>
        <div className="space-y-1">
          {threads?.slice(0, 5).map((t, i) => (
            <div key={i} className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-cyan-300 tabular-nums ml-auto">{t.actual_hz?.toFixed(0)}Hz</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Engines */}
      <div>
        <h3 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 font-semibold">Engines</h3>
        <div className="space-y-1.5">
...
function DataRow({ label, value }) {
  return (
    <div className="flex justify-between items-center">
      ...
      <span className="text-[11px] text-[#33CCFF] font-bold tabular-nums">{value}</span>
    </div>
  );
}
